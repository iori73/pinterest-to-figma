...
## [2026-09-06] 認証なし公開プロキシは「ホスト検証済みで安全」と思っても redirect: 'follow' で崩れる

Pinterest to Figma の Cloudflare Worker（worker/pinterest-proxy.js）は、対象URLのホストを
正規表現で検証してから `fetch()` していたが、そのfetchが `redirect: 'follow'`（デフォルト）だったため、
「検証したのは最初のURLだけ」で、fetch先が途中でリダイレクトした場合の行き先ホストは一切検証していなかった。
このWorkerは認証なしで誰でも直接叩ける公開エンドポイントなので、もし対象サイトが（意図的・非意図的問わず）
クロスホストにリダイレクトする振る舞いを持っていたら、このWorkerはそのまま任意サイトへの
オープンプロキシとして悪用され得る状態だった。

**教訓**: 「ユーザー入力のURLのホストをallowlistで検証してからfetchする」設計は、
`fetch()` がリダイレクトを自動フォローする限り不完全。`redirect: 'manual'` にして
リダイレクト先を都度自分で検証してからでないと安全とは言えない。
これは「入力を検証した」ことと「実際にネットワークアクセスする先を検証した」ことが
イコールではないケースの典型で、外部URLを中継するプロキシ/リレー系の実装をする際は必ず確認すること。

同じセッションで、サムネイルサイズを固定するためのパス書き換え処理（正規表現による先頭セグメント置換）が
二重スラッシュを含むパスでバイパスされる別のバグも発見した。**教訓**: パス操作を正規表現の置換だけで
行うと、「マッチしなかった場合は元の文字列がそのまま返る」という挙動が原因で、意図した書き換えが
サイレントに無効化されるパターンが起こり得る。文字列操作で安全性を担保したい場合は、
「マッチしなければ失敗する」設計（分割→強制的に上書き→再結合、など）にする方が、
正規表現の非マッチによるサイレントなバイパスを防げる。
